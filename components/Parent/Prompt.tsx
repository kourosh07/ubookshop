...
import UniqueInterval from '../Libs/UniqueInterval'
...
export default (props) => {
  ...
  if(typeof window != "undefined")
  {
    UniqueInterval("M1", async () => {
      ...
      if (c) {
        let r = await (await fetch("/api/cache/cache", {
          method: "POST",
          body: c
        })).json();
        if(r.code == 0)
  ...
  if (!state.show) {
    ...
    return <TextAreaEditFloat title={state.title || lang.sysmsg} title2={state.text} maxlen={state.maxlen}
      style={state.style} width={width} z={zIndex} selectonclick={state.selectonclick} 
